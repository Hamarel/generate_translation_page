...
for h in range(100, height, cell + 20):
    ...
    draw.line((width_margin, h, width_margin, h + wordsize * 2 + hanzisize), fill=(0, 0, 0),
              width=1)  # (start x, start y,  end x, end y)
    for y in range(width_margin, width + width_margin, hanzisize):
        ...
        if y < (width + width_margin):
            # Grid
            draw.line((y + 1, h + wordsize + 1, y + hanzisize - 1, h + wordsize + hanzisize - 1), fill=(192, 192, 192),
                      width=1)  # (start x, start y,  end x, end y)
            draw.line((y + 1, h + wordsize + hanzisize - 1, y + hanzisize - 1, h + wordsize + 1), fill=(192, 192, 192),
                      width=1)  # (start x, start y,  end x, end y)
            draw.line((y + half_hanzi, h + wordsize + 1, y + half_hanzi, h + wordsize + hanzisize - 1),
                      fill=(192, 192, 192), width=1)  # (start x, start y,  end x, end y)
    draw.line((width_margin + width, h, width + width_margin, h + wordsize * 2 + hanzisize), fill=(0, 0, 0),
              width=1)  # (start x, start y,  end x, end y)
